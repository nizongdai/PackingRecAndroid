...
import ltd.zejin.packingrec.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private var pendingTrimTailUs: Long = 0L
    private var pendingTrimPath: String? = null
    ...
    private fun startCamera() {
        ...
        cameraProviderFuture.addListener({
            ...
            cameraSelector = selectWideCameraSelector(cameraProvider)
    ...
    private fun selectWideCameraSelector(cameraProvider: ProcessCameraProvider): CameraSelector {
        var bestInfo: CameraInfo? = null
        ...
        cameraProvider.availableCameraInfos.forEach { info ->
            ...
            val focals = camera2Info.getCameraCharacteristic(
                CameraCharacteristics.LENS_INFO_AVAILABLE_FOCAL_LENGTHS
            )
            val minFocal = focals?.minOrNull()
            if (minFocal != null && minFocal < bestFocal) {
                bestFocal = minFocal
                bestInfo = info
            }
        }
        val chosen = bestInfo ?: return CameraSelector.DEFAULT_BACK_CAMERA
    ...
    private fun createTimestampOverlayEffect(): OverlayEffect {
        ...
        effect.setOnDrawListener { frame ->
            @Suppress("USELESS_ELVIS_OPERATOR")
    ...
    private fun handleBarcodeResult(text: String, timestamp: Long, hasMotion: Boolean) {
        ...
        // 自动停止：中央区域画面持续无变化达到停止空置秒数，且当前正在录制
            ...
            stopRecording(false, (settings.stopEmptySeconds * 1_000_000L).toLong())
        } else if (isRecording && recordingStartAt > 0L &&
            timestamp - recordingStartAt >= settings.maxRecordingSeconds * 1000L) {
            stopRecording(false)
        }
        ...
        recording = currentVideoCapture.output
            ...
            .start(ContextCompat.getMainExecutor(this)) { event ->
                if (event is VideoRecordEvent.Finalize) {
                    isRecording = false
                    recording = null
                    handleRecordingFinalize(file, fileBaseName)
    ...
    private fun stopRecording(userInitiated: Boolean, trimTailUs: Long = 0L) {
        if (recording == null) {
            return
        }
        if (trimTailUs > 0L) {
            pendingTrimTailUs = trimTailUs
            pendingTrimPath = currentRecordingFile?.absolutePath
        } else {
            pendingTrimTailUs = 0L
            pendingTrimPath = null
        }
        recording?.stop()
        recording = null
        isRecording = false
    ...
    private fun enqueuePendingUploads() {
        ...
        WorkManager.getInstance(this).cancelUniqueWork("ftp_upload_queue")
        files.forEach { file ->
    ...
    private fun requestRecordingSwitch(newBarcodeText: String) {
        ...
        stopRecording(false, 2_000_000L)
    }

    private fun handleRecordingFinalize(file: File, baseName: String) {
        val trimTailUs = pendingTrimTailUs
        if (trimTailUs > 0L && pendingTrimPath == file.absolutePath) {
            pendingTrimTailUs = 0L
            pendingTrimPath = null
            trimExecutor.execute {
                val trimmed = VideoOverlapProcessor.trimTail(file, trimTailUs)
                if (!trimmed) {
                    enqueueFtpUploadIfEnabled(file, baseName)
                } else if (file.exists()) {
                    enqueueFtpUploadIfEnabled(file, baseName)
                }
            }
    ...
    private fun scheduleLocalVideoCleanup() {
        val workRequest = PeriodicWorkRequestBuilder<LocalVideoCleanupWorker>(1, java.util.concurrent.TimeUnit.DAYS)
            .build()
